...
const handleInscription = async (formationId, titre, prix) => {
  const reference = prompt(`Pour valider votre inscription à "${titre}", envoyez ${prix} FCFA par Orange Money au +226 XX XX XX XX.\n\nEntrez ici la RÉFÉRENCE de la transaction (ex: PP2603...) :`);
  ...
  const { error } = await supabase.from('inscriptions').insert([
    { 
      ...
      mode_paiement: 'Orange Money' // Tu peux dynamiser ceci plus tard
    }
  ])
